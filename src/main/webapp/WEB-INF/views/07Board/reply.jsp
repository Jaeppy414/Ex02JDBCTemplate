...
<body>
	...
	<div class="container">
		...
		<!-- 
		답글쓰기는 새로운 게시물을 작성하는 것이므로 패스워드 입력이 있어야 한다.
		그리고 원본글을 가져오되 답변에 관련된 약간의 문자열을 추가해서 폼에 입력한다.(제목 => [RE], 내용 => [원본글])
		 -->
		<form name="writeFrm" method="post" action="./replyAction.do"
			onsubmit="return checkValidate(this);">
			
			<!-- 답변글 처리를 위해 원본글에 대한 idx값과 추가적으로 bgroup, bstep, bindent값이 필요하다.
			해당 값을 통해 게시물을 하나의 그룹으로 묶어주고, 그룹 내에서 정렬 및 목록 출력시 들여쓰기 처리를 하게 된다. -->
			...
			<table border=1 width=800>
				...
				<tr>
					<td>제목</td>
					<td><input type="text" name="title" style="width: 90%;" value="${replyRow.title }"/></td>
				</tr>
				<tr>
					<td>내용</td>
					<td><textarea name="contents"
							style="width: 90%; height: 200px;">${replyRow.contents }</textarea></td>
				</tr>
				<tr>
