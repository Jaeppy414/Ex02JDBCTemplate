...
List update(String sql)
	: update, delete, insert 쿼리문을 실행한다. 행의 변화가 생기므로
...
public class JDBCTemplateDAO {
	/*
	 * 멤버변수는 DB연결과 Spring-JDBC사용을 위해 선언한다. 컨트롤러에서 @Autowired를 통해 자동주입받은 빈을 정적변수인
	 * JdbcTemplateConst.template에 할당하였으므로, DB작업을 DAO 에서도 수행할 수 있다.
	 */
	...
	public ArrayList<SpringBoardDTO> list(Map<String, Object> map) {
		...
		/*
		 * RowMapper가 select를 통해 얻어온 ResultSet을 갯수만큼 반복하여 DTO에 저장한 후 List컬렉션에 추가하여
		 * 반환해준다. 그러므로 DAO에서 개발자가 반복적으로 작성했던 코드를 생략할 수 있다.
		 */
	...
	public int write(SpringBoardDTO SpringBoardDTO) {
		 ...
		 * PreparedStatementCreator()를 익명클래스로 정의하여 메서드를 오버라이딩 한 후 인파라미터가 있는 쿼리문에 값을 설정한
		 * 후 실행한다.
		 */
		int result = template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				이 때 nextval을 한문장에서 두번 사용하게 되는데, 한 문장에서 두 번 나오더라도
				항상 하나의 시퀀스만 반환하게 된다.
				 */
				...
				// 설정 후 완성된 쿼리문을 가진 psmt객체를 반환하면 update()
				// 메서드가 실행한 후 결과값을 반환한다.
				return psmt;
			}
		});
	...
	public void updateHit(String idx) {
		...
		/*
		update()메서드는 int형의 반환값이 있지만 필요없는 경우
		생략할 수 있다. 여기서는 필요하지 않아 void로 선언되어 있다.
		 */
	}
	//일련번호에 해당하는 게시물을 인출
	public SpringBoardDTO view(String idx) {
		...
		//인파라미터가 있는 select쿼리문을 작성한다.
		String sql = "SELECT * FROM springboard "
				+ " WHERE idx=?";
		/*
		queryForObject()메서드는 쿼리문을 실행한 후 반드시 하나의 결과를 반환해야 한다.
		그렇지 않으면 예외가 발생하므로 반드시 예외처리를 해주는 것이 좋다.
		 */
		try {
			/*
			...
			dto = template.queryForObject(sql, 
					...
					new Object[] {idx});
			/*
			BeanPropertyRowMapper객체는 쿼리의 실행결과를 DTO에 저장해주는 역할을 한다.
			이 때 테이블의 컬럼명과 DTO의 멤버변수명이 일치해야 한다.
			테이블의 컬럼명, 작성폼의 name속성값, DTO의 멤버변수를 모두 하나의 이름으로
			일치시켜야 한다.
			 */
		} 
		catch (Exception e) {
	...
	public int password(String idx, String pass) {
		...
		try {
			/*
			패스워드와 일련번호의 조건에 만족하는 레코드가 없는 경우
			예외가 발생하게 되므로 반드시 예외처리를 해야한다.
			 */
	...
	public void edit(SpringBoardDTO dto) {
		//인파라미터가 있는 update쿼리문 작성
		String sql = "UPDATE springboard "
				+ " SET name=?, title=?, contents=?"
				+ " WHERE idx=? AND pass=?";
		//update() 메서드의 2번쨰 인수로 Object형 배열을 사용한다.
	...
	public void delete(String idx, String pass) {
		...
		//PreparedStatementSetter 익명클래스를 통해 인파라미터를 설정한 후 쿼리문을 실행한다.
		//update()메서드는 정수값을 반환하지만 필요없는 경우 사용하지 않는다.
		template.update(sql, new PreparedStatementSetter() {
	...
	public void reply(final SpringBoardDTO dto) {
		...
		//답변글 추가 전에 bstep(그룹 내의 정렬)을 일괄적으로 업데이트한다.
		replyPrevUpdate(dto.getBgroup(), dto.getBstep());
		
		/*
		글쓰기의 경우 원본글이므로 idx와 bgroup은 같은 값을 입력한다.
		하지만 답변글은 원본글을 기반으로 작성되므로 idx는 새로운 시퀀스를 사용하면 되고,
		...
		template.update(sql, new PreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps) throws SQLException {
				...
				/*
				답변글은 원본글 아래쪽에 노출되어야 하고, 또한 들여쓰기도 되어야 한다.
				따라서 원본글에 +1 해준 후 입력해야 한다.
				 */
				//스탭은 게시물의 그룹내에서의 정렬순서를 의미한다.
				ps.setInt(6, dto.getBstep()+1);
				//인덴트는 들여쓰기 즉 depth를 의미한다.
	...
	원본글에 답변을 2번 이상 작성하는 경우 step에 입력되는 값이 동일해지는
	현상을 방지하기 위한 목적으로 업데이트하는 것이다.
	 */
