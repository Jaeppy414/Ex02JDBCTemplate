...
public class BoardController {
	/*
	 * @Autowired : 스프링 컨터이너에 미리 생성되어있는 빈을 자동으로 주입받을때 사용한다. 자료형을 기반으로 자동주입되므로 만약
	 * JdbcTemplate타입의 빈이 생성되어 있지 않다면 에러가 발생하게된다. 해당 어노테이션은 setter(), 멤버변수 등에서 사용할 수
	 * 있다.
	 */
	...
	public void setTemplate(JdbcTemplate template) {
		...
		/*
		 * template을 다른 클래스에서 사용하기 위해 static으로 선언한 변수에 할당한다. static(정적)변수는 프로그램 시작시 로딩되어
	...
	/*
	 * 멤버변수로 선언하여 클래스에서 전역적으로 사용할 수 있다. 게시판 구현을 위한 모든 Service객체는 해당 인터페이스를 구현하여 정의할
	 * 것이다.
	 */
	...
	public String list(Model model, HttpServletRequest req) {
		...
		/*
		 * 사용자로부터 받은 모든 요청은 request객체에 저장되고, 이를 ListExecute객체(Service객체)로 전달하기 위해
		 * Model객체에 저장한 후 매개변수로 전달한다.
		 */
	...
	/*
	 * 글쓰기 처리에 대한 매핑처리를 한다. 전송방식은 post로 전송시 요청명은 value 속성에 기술한다.
	 */
	@RequestMapping(value = "/board/writeAction.do", method = RequestMethod.POST)
	public String writeAction(Model model, HttpServletRequest req, SpringBoardDTO SpringBoardDTO) {
		/*
		 * 글쓰기 페이지에서 전송된 모든 폼값은 DTO객체를 통해 한꺼번에 받을수있다. Spring에서 이와같은 객체를 '커맨드객체'라고 한다.
		 * 따라서 폼값을 받기위해 getParamter()메서드를 일일히 사용할 필요가 없어진다.
		 */
	...
	public String view(Model model, HttpServletRequest req) {
		/*
		목록에서 제목을 클릭할 때 전달되는 파라미터는 idx, pageNum등을 저장한 request내장객체를 모델객체에 저장한다.
		 */
		model.addAttribute("req", req);
	...
	public String password(Model model, HttpServletRequest req) {
		...
		받은 후 모델객체에 저장한다, 나머지 값은 View에서 직접 EL을 통해 받을 것이다.
		 */
	...
	public String editAction(HttpServletRequest req, Model model, SpringBoardDTO SpringBoardDTO) {
		...
		/*
		수정처리가 완료되면 상세페이지로 이동하게 되는데 이때 idx와 같은 파라미터가 필용하다.
		Model객체에 저장한 후 redirect하면 자동으로 쿼리스트링으로 요청명 뒤에 추가하게 된다.
		 */
		model.addAttribute("idx", req.getParameter("idx"));
		model.addAttribute("nowPage", req.getParameter("nowPage"));
	...
	public String replyAction(HttpServletRequest req, Model model, SpringBoardDTO SpringBoardDTO) {
