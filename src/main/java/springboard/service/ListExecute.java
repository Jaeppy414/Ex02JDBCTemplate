...
오버라이딩 해야한다. 또한 해당 객체는 부모객체인 IBoardService 객체로
참조할 수 있다. 
 */
public class ListExecute implements IBoardService {
	...
	public void execute(Model model) {
		...
		/*
		컨트롤러에서 인수로 전달한 Model객체에는 request객체가 저장되어
		있다. asMap() 메서드를 통해 Map컬렉션으로 변환한 후 모든 요청을
		얻어올 수 있다. 
		 */
		Map<String, Object> paramMap = model.asMap();
		/*
		Model객체에 저장될때는 Object타입으로 변환되어 저장되므로, 
		사용을 위해 원래의 타입으로 형변환 해야한다. Model객체는
		4가지 영역과 동일한 특성을 가진다. 
		 */
...
/** 페이지 처리 start */
		...
		//게시판에서 한페이지 출력할 게시물 수, 한 블럭당 표시할 페이지번호 갯수
		...
		//전체페이지수를 계산한다.(전체게시물수/페이지당 갯수)
		int totalPage = (int)Math.ceil((double)totalRecordCount/pageSize);
		
		//파라미터로 전달되는 페이지 번호를 읽어온다.
